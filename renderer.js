...
document.querySelector('#audio_file_pane').ondrop = function(ev){
    ...
    ffmpeg.ffprobe(afPath,function(err, metadata) {
        if(err){
...
document.querySelector('#mux_button').onclick = function(ev){
...
document.querySelector('#reset_button').onclick = function(ev){
        vfPath = null;
        afPath = null;
        outputPath = null;  
        frameCount = 0;
...
document.querySelector('#create-new-file').onclick = function(ev){

    outputPath = dialog.showSaveDialog({
        title:'Save As',
        defaultPath:'~/Desktop/output.mp4'      
    })
